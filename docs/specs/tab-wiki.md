...
8. **Partager** : `navigator.share` ou fallback clipboard avec URL `#wiki/{id}` (mais le router ne consomme pas ce hash — cf. limitations).

## Fonctionnalités
- **Recherche client full-text** : sur 3 champs (title, excerpt, tags), case-insensitive, avec highlight `<mark>` dans le titre et l'excerpt des résultats.
- **3 filtres combinables** : Catégorie (auto-détectée depuis les concepts du corpus, préfixée "Tout"), Source (`all / auto / perso` — **`perso` n'a aucun effet** car le loader set `kind: "auto"` pour toutes les entrées DB), Tri (Récent / Plus lus / A-Z).
- **Épinglage persistent** : clé `wiki-pinned-ids` dans localStorage (Set d'ids). Section "Épinglées · accès rapide" visible uniquement quand aucun filtre n'est actif.
- **Vue détail avec markdown-lite** : parser custom dans le panel (pas de lib externe). Supporte H1 `# ` H2 `## `, quote `> `, liste `- ` ou `1. `, et inline `**bold**` / `*italic*`. **Pas** de code blocks, pas de liens, pas d'images.
- **TOC auto** : scan des H1/H2 du contenu rendu, liens cliquables avec `scrollIntoView` smooth.
- **Related + Backlinks** : liens en double-sens calculés au loader via `related_concepts` JSONB. `related` affiche ce que l'entrée cite, `backlinks` affiche ce qui cite l'entrée.
- **Demander à Jarvis** : prompt préfilé (détail) ou formulaire complet (create) → `localStorage.jarvis-prefill-input` + `onNavigate("jarvis")`.
- **Partager** : URL pseudo-deep-link `#wiki/{id}`. `navigator.share` en priorité, fallback clipboard.
...
- **URL share** : `#wiki/{id}` est écrite dans le clipboard mais **le router `app.jsx` ne consomme pas ce hash format** — un utilisateur qui clique sur le lien atterrit sur le wiki hub, pas sur l'entrée. Hash géré seulement pour les route ids simples.
...
- **kind filter "perso"** : le toggle existe mais aucune entrée ne répond (toutes sont `kind: "auto"`) → liste vide silencieuse.
