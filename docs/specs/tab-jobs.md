...
## Dernière MAJ
2026-04-29 — fix front : transformers élargis pour absorber l'hétérogénéité des schémas écrits par la routine Cowork. `transformJobRubric` accepte maintenant 3-axis EN/FR/abrégé (`{seniority/sector/impact}`, `{seniorite/secteur/impact}`, `{sen/sec/imp}`, plus `bonus`) et les fallbacks single-axis (`role` mappé via dictionnaire de slugs, `note`, `reason`, `redflag`/`red_flag`, `gap`). Couverture passée de 28 % (85/300) à 85 % (254/300) des cards avec une justif visible. `signal_cv` lit la nouvelle forme V2 (`pdf_recommended_count`, `docx_recommended_count`, `implication`) en plus de l'ancienne (`pdf_pct`, `insight`). Bug TZ corrigé sur le calcul du lundi : on construit l'iso depuis les composantes locales au lieu de `toISOString()` UTC qui décalait les barres d'un jour.
