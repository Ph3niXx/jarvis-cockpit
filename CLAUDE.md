...
- **CI `lint-specs`** ([.github/workflows/lint-specs.yml](.github/workflows/lint-specs.yml)) — **bloquant** : fail toute PR qui introduit du vocabulaire technique (chemins `.jsx/.py`, composants JSX, globals `data.xxx`/`window.X`/`X_DATA`, colonnes DB `xxx_id/_at/_html`, endpoints `/rest/v1/...`, props `key=value`) dans une section `## Fonctionnalités`. Règle éditoriale dure : les détails d'implémentation appartiennent à `Front — structure UI` et `Back — sources de données`, pas au bullet produit. Script : [scripts/lint_specs_fonctionnalites.py](scripts/lint_specs_fonctionnalites.py).
